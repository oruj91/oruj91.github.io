...
const changeActiveBtn = function() {
    ...
    const buttons = document.querySelectorAll('.left-navigation-btn');

    container.addEventListener('click', (e) => {
        e.preventDefault();
        
        if (e.target.classList.contains('left-navigation-btn')) {
...
// Chart #1
var ctx = document.getElementById("activePlayers").getContext('2d');
var myChart = new Chart(ctx, {
    type: 'line',
    data: {
        labels: ["MON","TUE","WED","THU","FR","SAT","SUN"],
        datasets: [{
            data: [40,25,32,50,30,28,40,20],
            backgroundColor: ['rgba(32, 128, 126, 0.05)'],
...
// Chart #2
var ctx = document.getElementById("tournamentsChart").getContext('2d');
var myChart = new Chart(ctx, {
    type: 'line',
    data: {
        labels: ["MON","TUE","WED","THU","FR","SAT","SUN"],
        datasets: [{
            data: [450,750,600,900,460,600,670,300],
...
changePercentageColor = function(percentage) {
    let color;
    if (percentage >= 75) {            
        color = '#8FC46E';
    } else {
...
// let newPlayersPercentage = 58;
let newPlayersPercentage = document.getElementById('newPlayersPerc').textContent;
let revistedPercentage = document.getElementById('revistedPerc').textContent;
let commentedPercentage = document.getElementById('commentedPerc').textContent;
let activePlayersPercentage = document.getElementById('activePlayersPerc').textContent;

changePercentageColor(newPlayersPercentage);
changePercentageColor(revistedPercentage);
changePercentageColor(commentedPercentage);
changePercentageColor(activePlayersPercentage);

// New Players
var ctx = document.getElementById("newPlayersStatistics");
var myChart = new Chart(ctx, {
    type: 'doughnut',
    data: {
        datasets: [{
            data: [newPlayersPercentage, (100 - newPlayersPercentage)],
            backgroundColor: [changePercentageColor(newPlayersPercentage), '#D1D3D4'],
            borderWidth: 0
        }]
    },
    options: {
...
// Revisted
var ctx = document.getElementById("revistedStatistics");
var myChart = new Chart(ctx, {
    type: 'doughnut',
    data: {
        labels: false,
        datasets: [{
            data: [revistedPercentage, (100 - revistedPercentage)],
            backgroundColor: [changePercentageColor(revistedPercentage), '#D1D3D4'],
            borderWidth: 0
        }]
    },
    options: {
...
// Commented
var ctx = document.getElementById("commentedStatistics");
var myChart = new Chart(ctx, {
    type: 'doughnut',
    data: {
        labels: false,
        datasets: [{
            data: [commentedPercentage, (100 - commentedPercentage)],
            backgroundColor: [changePercentageColor(commentedPercentage), '#D1D3D4'],
            borderWidth: 0
        }]
    },
    options: {
...
// Active Players
var ctx = document.getElementById("activePlayersStatistics");
var myChart = new Chart(ctx, {
    type: 'doughnut',
    data: {
        labels: false,
        datasets: [{
            data: [activePlayersPercentage, (100 - activePlayersPercentage)],
            backgroundColor: [changePercentageColor(activePlayersPercentage), '#D1D3D4'],
            borderWidth: 0
        }]
    },
    options: {
...
// Inner Sidebar
new SimpleBar(document.getElementById('simplebar-container'), {autoHide: false});